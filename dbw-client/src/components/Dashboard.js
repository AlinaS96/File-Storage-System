...
//Dashboard component is working as homeScreen for the Application frontend
const Dashboard = () => {
          ...
          <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
            ...
            <div className="px-4 py-6 sm:px-0">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="flex w-full pl-3">
                <input type="file" {...register('file')} className="border-[1px] border-solid border-[#ccc]" multiple={true}/>
                <button className="bg-gray-800 text-white w-20 rounded-md ml-2 hover:bg-gray-700">Submit</button>
              </form>
              </div>
              {files && files.map((file, key) => (
                <div key={key}>
                    <UploadedFileCard fileName={file.file_name} fileSize={Math.round(file.file_size)} ownerId={file.owner_id} ownerName={file.owner_name} fileHash={file.file_hash} uploadTime={file.upload_time} blockedStatus={file.blocked_status}/>
                </div>
              ))}
              </div>
            </div>
